...
static DrawCommand* layerAllocDrawCommand(Context* ctx, Layer* layer, uint32_t numVertices, uint32_t numIndices, DrawCommand::Type::Enum type, uint16_t handle);
static DrawCommand* layerAllocClipCommand(Context* ctx, Layer* layer, uint32_t numVertices, uint32_t numIndices);
...
static void submitCachedMesh(Context* ctx, Layer* layer, Color col, const CachedMesh* meshList, uint32_t numMeshes);
static void submitCachedMesh(Context* ctx, Layer* layer, GradientHandle gradientHandle, const CachedMesh* meshList, uint32_t numMeshes);
static void submitCachedMesh(Context* ctx, Layer* layer, ImagePatternHandle imgPatter, Color color, const CachedMesh* meshList, uint32_t numMeshes);
#endif
...
static void ctxIndexedTriList(Context* ctx, const float* pos, const uv_t* uv, uint32_t numVertices, const Color* colors, uint32_t numColors, const uint16_t* indices, uint32_t numIndices, ImageHandle img)
{
	...
	DrawCommand* cmd = layerAllocDrawCommand(ctx, layer, numVertices, numIndices, DrawCommand::Type::Textured, img.idx);
...
static void createDrawCommand_VertexColor(Context* ctx, Layer* layer, const float* vtx, uint32_t numVertices, const uint32_t* colors, uint32_t numColors, const uint16_t* indices, uint32_t numIndices)
{
	...
	DrawCommand* cmd = layerAllocDrawCommand(ctx, layer, numVertices, numIndices, DrawCommand::Type::Textured, fontImg.idx);
...
{
	DrawCommand* cmd = layerAllocDrawCommand(ctx, layer, numVertices, numIndices, DrawCommand::Type::ImagePattern, imgPatternHandle.idx);
...
{
	DrawCommand* cmd = layerAllocDrawCommand(ctx, layer, numVertices, numIndices, DrawCommand::Type::ColorGradient, gradientHandle.idx);
...
{
	// Allocate the draw command
	DrawCommand* cmd = layerAllocClipCommand(ctx, layer, numVertices, numIndices);
...
static DrawCommand* layerAllocDrawCommand(Context* ctx, Layer* layer, uint32_t numVertices, uint32_t numIndices, DrawCommand::Type::Enum type, uint16_t handle)
{
	...
	if (layer->m_NumDrawCommands == layer->m_DrawCommandCapacity) {
		...
		VG_CHECK(layer->m_DrawCommands != nullptr, "Failed to allocate draw commands");
	}

	const uint32_t cmdID = layer->m_NumDrawCommands;
	layer->m_NumDrawCommands++;

	DrawCommand* cmd = &layer->m_DrawCommands[cmdID];
...
static DrawCommand* layerAllocClipCommand(Context* ctx, Layer* layer, uint32_t numVertices, uint32_t numIndices)
{
	...
	if (layer->m_NumClipCommands == layer->m_ClipCommandCapacity) {
		...
		VG_CHECK(layer->m_ClipCommands != nullptr, "Failed to allocate clip commands");
	}

	const uint32_t cmdID = layer->m_NumClipCommands;
	layer->m_NumClipCommands++;

	DrawCommand* cmd = &layer->m_ClipCommands[cmdID];
	cmd->m_VertexBufferHandle = vbHandle;
	cmd->m_FirstVertexID = firstVertexID;
	cmd->m_FirstIndexID = firstIndexID;
	cmd->m_NumVertices = 0;
...
static void renderTextQuads(Context* ctx, Layer* layer, uint32_t numQuads, Color color)
{
	...
	DrawCommand* cmd = layerAllocDrawCommand(ctx, layer, numDrawVertices, numDrawIndices, DrawCommand::Type::Textured, ctx->m_FontImages[ctx->m_FontImageID].idx);
...
static void clCacheRender(Context* ctx, Layer* layer, CommandList* cl)
{
	...
	while (cmd < cmdListEnd) {
		...
		case CommandType::FillPathColor: {
			...
			const Color color = CMD_READ(cmd, Color);
			BX_UNUSED(flags);
			submitCachedMesh(ctx, layer, color, &clCache->m_Meshes[nextCachedCommand->m_FirstMeshID], nextCachedCommand->m_NumMeshes);
			++nextCachedCommand;
		} break;
		case CommandType::FillPathGradient: {
			const uint32_t flags = CMD_READ(cmd, uint32_t);
			...
			const GradientHandle gradient = { isLocal(gradientFlags) ? (uint16_t)(gradientHandle + firstGradientID) : gradientHandle, 0 };
			submitCachedMesh(ctx, layer, gradient, &clCache->m_Meshes[nextCachedCommand->m_FirstMeshID], nextCachedCommand->m_NumMeshes);
			++nextCachedCommand;
		} break;
		case CommandType::FillPathImagePattern: {
			const uint32_t flags = CMD_READ(cmd, uint32_t);
			...
			const ImagePatternHandle imgPattern = { isLocal(imgPatternFlags) ? (uint16_t)(imgPatternHandle + firstImagePatternID) : imgPatternHandle, 0 };
			submitCachedMesh(ctx, layer, imgPattern, color, &clCache->m_Meshes[nextCachedCommand->m_FirstMeshID], nextCachedCommand->m_NumMeshes);
			++nextCachedCommand;
		} break;
		case CommandType::StrokePathColor: {
			const float width = CMD_READ(cmd, float);
...
static void submitCachedMesh(Context* ctx, Layer* layer, ImagePatternHandle imgPattern, Color col, const CachedMesh* meshList, uint32_t numMeshes)
{
	VG_CHECK(!layer->m_RecordClipCommands, "Only submitCachedMesh(Color) is supported inside BeginClip()/EndClip()");

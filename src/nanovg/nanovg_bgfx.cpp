...
#include "fs_nanovg_fill.bin.h"
	...
	struct GLNVGcontext
	{
		...
		bgfx::DynamicVertexBufferHandle activeDVB;
		uint8_t m_viewId;
		...
		int cpaths;
		int npaths;
		struct GLNVGvertexBuffer* vertexBuffers;
		int cvertexBuffers;
		int nvertexBuffers;
	...
	static int nvgRenderCreate(void* _userPtr)
	{
		...
		gl->texMissing = bgfx::createTexture2D(4, 4, false, 0, bgfx::TextureFormat::BGRA8, 0, mem);
	...
	static void glnvg__fill(struct GLNVGcontext* gl, struct GLNVGcall* call)
	{
		...
		for (i = 0; i < npaths; i++)
		{
			if (2 < paths[i].fillCount)
			{
				...
				bgfx::setVertexBuffer(0, gl->activeDVB);
		...
		if (gl->edgeAntiAlias)
		{
			...
			for (i = 0; i < npaths; i++)
			{
				...
				bgfx::setVertexBuffer(0, gl->activeDVB, paths[i].strokeOffset, paths[i].strokeCount);
				bgfx::setTexture(0, gl->s_tex, gl->th);
				bgfx::submit(gl->m_viewId, gl->prog);
			}
		}
		...
		// Draw fill
		bgfx::setState(gl->state);
		bgfx::setVertexBuffer(0, gl->activeDVB, call->vertexOffset, call->vertexCount);
		bgfx::setTexture(0, gl->s_tex, gl->th);
		bgfx::setStencil(0
...
// 			else
			...
			int prevVertexBufferID = -1;

			for (uint32_t ii = 0, num = gl->ncalls; ii < num; ++ii)
			{
				struct GLNVGcall* call = &gl->calls[ii];

				if (call->vertexBufferID != prevVertexBufferID) 
				{
					prevVertexBufferID = call->vertexBufferID;

					GLNVGvertexBuffer* vb = &gl->vertexBuffers[call->vertexBufferID];

					if (!bgfx::isValid(vb->dvb)) 
					{
		...
		// Reset calls
		gl->activeDVB = BGFX_INVALID_HANDLE;
		gl->nvertexBuffers = 0;
		gl->npaths    = 0;
		gl->ncalls    = 0;
		gl->nuniforms = 0;
	...
	static int glnvg__allocVertexBuffer(GLNVGcontext* gl)
	{
		if (gl->nvertexBuffers + 1 > gl->cvertexBuffers) 
		{
			int oldCount = gl->cvertexBuffers;
			gl->cvertexBuffers += 8;
			gl->vertexBuffers = (GLNVGvertexBuffer*)BX_REALLOC(gl->m_allocator, gl->vertexBuffers, sizeof(GLNVGvertexBuffer) * gl->cvertexBuffers);
			// TODO: Error checking.
			memset(&gl->vertexBuffers[oldCount], 0, sizeof(GLNVGvertexBuffer) * 8);
			for (int i = oldCount; i < gl->cvertexBuffers; ++i) 
			{
				gl->vertexBuffers[i].dvb = BGFX_INVALID_HANDLE;
			}
		}
...
//			vb->cverts = 65536;
//			vb->verts = (NVGvertex*)BX_ALLOC(gl->m_allocator, sizeof(NVGvertex) * vb->cverts);
		}

		vb->nverts = 0;

		return gl->nvertexBuffers - 1;
	}

	static int glnvg__allocVerts(GLNVGcontext* gl, int n, int& vertexBufferID)
	{
		BX_CHECK(n <= 65536, "Tried to allocate a single batch of %d vertices. Maximum allowed number of vertices per batch is 65536.", n);

		// Always allocate vertices from the last vertex buffer (TODO: explain).
		if (gl->nvertexBuffers == 0) 
		{
			vertexBufferID = glnvg__allocVertexBuffer(gl);
		}
		else 
		{
			vertexBufferID = gl->nvertexBuffers - 1;
		}

		GLNVGvertexBuffer* vb = &gl->vertexBuffers[vertexBufferID];
		if (vb->nverts + n > vb->cverts) 
		{
			// The current vertex buffer cannot hold the requested amount of vertices. Allocate a new one.
			vertexBufferID = glnvg__allocVertexBuffer(gl);
			vb = &gl->vertexBuffers[vertexBufferID];
		}

		BX_CHECK(vb->nverts + n <= vb->cverts, "Not enough memory to allocate %d vertices.", n);

		int offset = vb->nverts;
		vb->nverts += n;
								  ...
								  const float* bounds, const struct NVGpath* paths, int npaths)
	{
		...
		offset = glnvg__allocVerts(gl, maxverts, call->vertexBufferID);
		...
		for (i = 0; i < npaths; i++)
		{
			...
			if (path->nfill > 0)
			{
				...
				memcpy(&vb->verts[offset], path->fill, sizeof(struct NVGvertex) * path->nfill);
				offset += path->nfill;
			}
			...
			if (path->nstroke > 0)
			{
				...
				memcpy(&vb->verts[offset], path->stroke, sizeof(struct NVGvertex) * path->nstroke);
				offset += path->nstroke;
			}
		}
		...
		call->vertexCount = 6;
		quad = &vb->verts[call->vertexOffset];
									   ...
									   const struct NVGvertex* verts, int nverts)
	{
		...
		// Allocate vertices for all the paths.
		call->vertexOffset = glnvg__allocVerts(gl, nverts, call->vertexBufferID);
		call->vertexCount = nverts;

		GLNVGvertexBuffer* vb = &gl->vertexBuffers[call->vertexBufferID];

		memcpy(&vb->verts[call->vertexOffset], verts, sizeof(struct NVGvertex) * nverts);
	...
	static void nvgRenderDelete(void* _userPtr)
	{
		...
		for (int i = 0, num = gl->cvertexBuffers; i < num; ++i) 
		{
			...
			{
				bgfx::destroy(gl->vertexBuffers[i].dvb);
			}
		}

...
#define NVGrealloc(x, y) M_REALLOC(x, y)
#define NVGfree(x) M_FREE(x)
...
{
	if (c == NULL) return;
	if (c->points != NULL) NVGfree(c->points);
	if (c->paths != NULL) NVGfree(c->paths);
	if (c->verts != NULL) NVGfree(c->verts);
	NVGfree(c);
}

static NVGpathCache* nvg__allocPathCache(void)
{
	NVGpathCache* c = (NVGpathCache*)NVGmalloc(sizeof(NVGpathCache));
	if (c == NULL) goto error;
	memset(c, 0, sizeof(NVGpathCache));

	c->points = (NVGpoint*)NVGmalloc(sizeof(NVGpoint)*NVG_INIT_POINTS_SIZE);
	if (!c->points) goto error;
	c->npoints = 0;
	c->cpoints = NVG_INIT_POINTS_SIZE;

	c->paths = (NVGpath*)NVGmalloc(sizeof(NVGpath)*NVG_INIT_PATHS_SIZE);
	if (!c->paths) goto error;
	c->npaths = 0;
	c->cpaths = NVG_INIT_PATHS_SIZE;

	c->verts = (NVGvertex*)NVGmalloc(sizeof(NVGvertex)*NVG_INIT_VERTS_SIZE);
...
NVGcontext* nvgCreateInternal(NVGparams* params)
{
	...
	NVGcontext* ctx = (NVGcontext*)NVGmalloc(sizeof(NVGcontext));
	int i;
	...
	ctx->commands = (float*)NVGmalloc(sizeof(float)*NVG_INIT_COMMANDS_SIZE);
...
void nvgDeleteInternal(NVGcontext* ctx)
{
	int i;
	if (ctx == NULL) return;
	if (ctx->commands != NULL) NVGfree(ctx->commands);
...
// JD: 
NVGcolor nvgRGBA32(unsigned int abgr)
{
	NVGcolor color;
	color.r = (abgr & 0x000000FF) / 255.0f;
	color.g = ((abgr >> 8) & 0x000000FF) / 255.0f;
	color.b = ((abgr >> 16) & 0x000000FF) / 255.0f;
	color.a = ((abgr >> 24) & 0x000000FF) / 255.0f;
	return color;
}
...
bool nvgIntersectScissor(NVGcontext* ctx, float x, float y, float w, float h)
{
	NVGstate* state = nvg__getState(ctx);
	...
	// JD: Return false in case scissor rect is too small for bgfx to handle correctly
	return rect[2] >= 1.0f && rect[3] >= 1.0f;
}
...
static void nvg__appendCommands(NVGcontext* ctx, float* vals, int nvals)
{
	...
	if (ctx->ncommands+nvals > ctx->ccommands) {
		...
		commands = (float*)NVGrealloc(ctx->commands, sizeof(float)*ccommands);
...
static void nvg__addPath(NVGcontext* ctx)
{
	...
	if (ctx->cache->npaths+1 > ctx->cache->cpaths) {
		...
		paths = (NVGpath*)NVGrealloc(ctx->cache->paths, sizeof(NVGpath)*cpaths);
...
static void nvg__addPoint(NVGcontext* ctx, float x, float y, int flags)
{
	...
	if (ctx->cache->npoints+1 > ctx->cache->cpoints) {
		...
		points = (NVGpoint*)NVGrealloc(ctx->cache->points, sizeof(NVGpoint)*cpoints);
...
static NVGvertex* nvg__allocTempVerts(NVGcontext* ctx, int nverts)
{
	if (nverts > ctx->cache->cverts) {
		...
		verts = (NVGvertex*)NVGrealloc(ctx->cache->verts, sizeof(NVGvertex)*cverts);
...
#if USE_RECURSIVE_BEZIER_TESSELLATION
								 ...
								 int level, int type)
{
	...
	while (true) {
		...
		if ((d2 + d3)*(d2 + d3) <= ctx->tessTol * (dx*dx + dy*dy)) {
			nvg__addPoint(ctx, x4, y4, stackPtr == stack ? type : 0); // Only the last point gets the passed type. All others get 0.
...
#endif // USE_RECURSIVE_BEZIER_TESSELLATION
...
int nvgTextBreakLines(NVGcontext* ctx, const char* string, const char* end, float breakRowWidth, NVGtextRow* rows, int maxRows, unsigned int flags)
{
	NVGstate* state = nvg__getState(ctx);
	float scale = nvg__getFontScale(state) * ctx->devicePxRatio;
	float invscale = 1.0f / scale;
	...
	while (fonsTextIterNext(ctx->fs, &iter, &q)) {
		...
		switch (iter.codepoint) {
			...
			case 32:		// space 
				...
				if (flags & NVG_TBLF_SPACES_AS_CHARS) {
					type = NVG_CHAR;
				} else {
					type = NVG_SPACE;
				}
...
void nvgTextBoxBounds(NVGcontext* ctx, float x, float y, float breakRowWidth, const char* string, const char* end, float* bounds, unsigned int flags)
{
	NVGstate* state = nvg__getState(ctx);
	NVGtextRow rows[2];
	float scale = nvg__getFontScale(state) * ctx->devicePxRatio;
	...
	while ((nrows = nvgTextBreakLines(ctx, string, end, breakRowWidth, rows, 2, flags))) {
		for (i = 0; i < nrows; i++) {
			NVGtextRow* row = &rows[i];

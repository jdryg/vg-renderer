...
int textGlyphPositions(Context* ctx, uint32_t layerID, const TextConfig& cfg, float x, float y, const char* text, const char* end, GlyphPosition* positions, int maxPositions);
...
float measureText(Context* ctx, uint32_t layerID, FontHandle fontHandle, float fontSize, uint32_t alignment, float x, float y, const char* str, const char* end, float* bounds);
void measureTextBox(Context* ctx, uint32_t layerID, FontHandle fontHandle, float fontSize, uint32_t alignment, float x, float y, float breakWidth, const char* str, const char* end, float* bounds, uint32_t textboxFlags);
float getTextLineHeight(Context* ctx, uint32_t layerID, FontHandle fontHandle, float fontSize, uint32_t alignment);
int textBreakLines(Context* ctx, uint32_t layerID, FontHandle fontHandle, float fontSize, uint32_t alignment, const char* str, const char* end, float breakRowWidth, TextRow* rows, int maxRows, uint32_t flags);
int textGlyphPositions(Context* ctx, uint32_t layerID, FontHandle fontHandle, float fontSize, uint32_t alignment, float x, float y, const char* str, const char* end, GlyphPosition* positions, int maxPositions);
